#@+leo-ver=5-thin
#@+node:ekr.20210810102041.1: * @file scripts/wax_off.py
#@+<< docstring >>
#@+node:ekr.20210810102041.2: ** << docstring >>
"""
The "wax-off" utility.

Create stub files from existing annotation, then optionally remove all
...
--diff: True: print diffs of proposed changes.
        False: hange the input files.

**Tweaking the stub files**

The wax_off script knows little about python: it is just a pattern
matcher. wax_off moves def lines (and class lines) to the stub file with
the *existing* indentation. So mypy may complain about syntax errors in the
stub file.
